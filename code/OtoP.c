...
{ // read parameters from para.txt;
	...
	get_int(f, &_runmode);	//  read _runmode; 0, directly OtoP training; 1, directly PtoP training; 2, OtoP reading by loading weights from PtoP training results;
	...
	printf("runmode=%d\n", _runmode); // read _runmode; 0, directly OtoP training; 1, directly PtoP training; 2, OtoP reading by loading weights from PtoP training results;
...
{ // read runtime parameters from command line input;
	...
	for(i=1;i<argc;i++)
		{ if(strcmp(argv[i],"-seed")==0){ assert(atoi(argv[i+1])>=0); _seed=atol(argv[i+1]); i++; } // set random seed via '-seed' argument;
		  else if(strncmp(argv[i],"-runmode",8)==0){ assert((atoi(argv[i+1])==0)||(atoi(argv[i+1])==1)||(atoi(argv[i+1])==2)); _runmode=atoi(argv[i+1]); i++; } // set runmode: 0, directly OtoP training; 1, directly PtoP training; 2, OtoP reading by loading weights from PtoP training results;
...
void train(Net *net, ExampleSet *TrExm, ExampleSet *TeExm, int ticks, unsigned int totaliter, unsigned int rep, 
		FILE *f1, char *fName1, FILE *f2, char *fName2, FILE *f3, char *fName3, FILE *f4, char *fName4, 
...
{ // train the network and record the training error and accuracies;
	...
	for(iter=1;iter<=totaliter;iter++)
		  ...
		  if(_samp_method==0)
		  	{ if(count==rep)
				{ error=error/(float)count;	// calculate average error;
			  	  accuTr=getacu(net, TrExm, ticks, iter, f2, fName2, f5, fName5, f7, fName7);	// calculate training accuracy;
				  accuTe=getacu(net, TeExm, ticks, iter, f3, fName3, f6, fName6, f8, fName8);	// calculate testing accuracy;
				  ...
				  fprintf(f4, "%d\t%d", iter, TrExm->numExamples);	
				  for(i=0;i<TrExm->numExamples;i++)
...
{ // create file names, actual files, and file headers;
	if((_runmode==0)||(_runmode==2))
		{ // record connection weights of the network;
		  ...
		  setF(outF, subDirect, "output.txt", f1, "ITER\tErr\tAcuTr\tAcuTe\n", NULL, 0, NULL);
		...
		{ // record connection weights of the network;
		  ...
		  // record item-based training and testing accuracy;
		  setF(itemacuTrF, subDirect, "itemacu_tr_ptop.txt", f2, "ITER\tNoItem", "\tAcu%d", train_exm->numExamples, "\tAvg\n");
		  setF(itemacuTeF, subDirect, "itemacu_te_ptop.txt", f3, "ITER\tNoItem", "\tAcu%d", test_exm->numExamples, "\tAvg\n");
		  // record accumulative occurring frequency of training examples during training;
		  setF(trainfreqF, subDirect, "trainfreq_ptop.txt", f4, "ITER\tNoItem", "\tF%d", train_exm->numExamples, "\n");
		  // record output phonemes of training and testing examples;
		  setF(outPhonTrF, subDirect, "outphonTr_ptop.txt", f5, "ITER\tNoItem", "\tPhon%d", train_exm->numExamples, "\n");
		  setF(outPhonTeF, subDirect, "outphonTe_ptop.txt", f6, "ITER\tNoItem", "\tPhon%d", test_exm->numExamples, "\n");
		  // record output vectors of training and testing examples;
		  if(_recVec==1) 
			{ setF(outPhonTrVecF, subDirect, "outphonTrVec_ptop.txt", f7, "ITER\tNoItem", "\tPV%d", train_exm->numExamples, "\n");		  
		  	  setF(outPhonTeVecF, subDirect, "outphonTeVec_ptop.txt", f8, "ITER\tNoItem", "\tPV%d", test_exm->numExamples, "\n");
			}
		}
}
...
{ // free result file names and files;
	...
	// record item-based training and testing accuracy;
	free(*itemacuTrF); *itemacuTrF=NULL; *f2=NULL;
	free(*itemacuTeF); *itemacuTeF=NULL; *f3=NULL;	
	// record accumulative occurring frequency of training examples during training;
	free(*trainfreqF); *trainfreqF=NULL; *f4=NULL;
	// record output phonemes of training and testing examples;
	free(*outPhonTrF); *outPhonTrF=NULL; *f5=NULL;
	free(*outPhonTeF); *outPhonTeF=NULL; *f6=NULL;	
	// record output vectors of training and testing examples;
	if(_recVec==1) 
		{ free(*outPhonTrVecF); *outPhonTrVecF=NULL; *f7=NULL; 
		  free(*outPhonTeVecF); *outPhonTeVecF=NULL; *f8=NULL; 
		}
}
...
{ // store seed to seed.txt;
	...
	else if(_runmode==2) { fName=malloc((strlen("seed_PtoP_OtoP.txt")+1)*sizeof(char)); assert(fName!=NULL); strcpy(fName, "seed_PtoP_OtoP.txt"); }
...
{ // main function: initialize network, and train, and calculate parameters;
	...
	if((_runmode==0)||(_runmode==2))
		{ // scratch OtoP training; or OtoP training by loading weights from PtoP training network;
		  // 1) build network;	
		...
		{ // scratch PtoP training;
		  ...
		  // 2) load training and testing examples;
		  train_exm=load_examples(_exTrF_PtoP, _tick_PtoP); test_exm=load_examples(_exTeF_PtoP, _tick_PtoP);
		}
	
	// 3) crete result file names and file headers;
	setResF(subDirect, &weightF, &f1, &outF, &f2, &itemacuTrF, &f3, &itemacuTeF, &f4, &trainfreqF, 
		  &f5, &outPhonTrF, &f6, &outPhonTeF, &f7, &outPhonTrVecF, &f8, &outPhonTeVecF);
	
	// 4) train the network;
	if((_runmode==0)||(_runmode==2))
		{ printf("Start OtoP training!\n");
		  train(reading, train_exm, test_exm, _tick_OtoP, _iter, _rep, 
		  		f1, outF, f2, itemacuTrF, f3, itemacuTeF, f4, trainfreqF, 
		  		f5, outPhonTrF, f6, outPhonTeF, f7, outPhonTrVecF, f8, outPhonTeVecF);
		  printf("Done OtoP training!\n");
		}
	else if(_runmode==1)
		{ printf("Start PtoP training!\n");
		  train(reading, train_exm, test_exm, _tick_PtoP, _iter_ptop, _rep_ptop, 
		  		f1, outF, f2, itemacuTrF, f3, itemacuTeF, f4, trainfreqF, 
		  		f5, outPhonTrF, f6, outPhonTeF, f7, outPhonTrVecF, f8, outPhonTeVecF);
	...
	// 7) free train_exm and test_exm and reading; 
